...
def delete_generated_model(model_id: str) -> None:
    ...
    # 이 모델(model_id)에 남아있던 커스텀 표시 이름도 함께 지운다.
    # 그렇지 않으면 같은 알파벳(model_c 등)의 모델을 다시 만들었을 때
    # 예전에 지웠던 모델의 이름이 그대로 남아 보이게 된다.
    reset_model_display_name(
        normalized_id
    )
...
def reset_demo_models_once() -> None:
    ...
    Model D ~ Model Z까지 사용자가 생성한 임시 모델을 모두 지운다.
 
    Model C는 Model A/B와 동일하게 유지 대상이라
    이 초기화에서 제외한다.
    ...
    # Model C는 이제 Model A/B처럼 새로고침(새 세션)해도 유지되는
    # 모델로 취급한다. 자동 초기화 대상은 Model D부터 시작한다.
    for model in models:
        model_id = str(
            model.get("model_id", "")
        ).strip().lower()
    ...
    for letter_code in range(
        ...
        # Scenario 데이터가 이미 없어도(예전에 지워진 모델이어도)
        # model_display_names.json에는 예전 커스텀 이름이 남아있을 수
        # 있으므로, 세션이 새로 시작될 때 model_d~z는 항상 초기화한다.
        # (model_c는 유지 대상이라 이 루프 자체에 포함되지 않는다.)
        reset_model_display_name(
            model_id
        )
...
def render_model_create() -> None:
    ...
    if create_clicked:
        ...
        try:
            ...
            # 사용자가 입력한 모델 이름을 홈/상세 화면에서 실제로
            # 표시되는 이름(model_display_names.json)으로도 저장한다.
            # 이걸 빼먹으면 scenario.json에는 이름이 저장되어도
            # 화면에는 예전 이름(또는 자동 생성된 "Model C" 등)이
            # 계속 보이게 된다.
            save_model_display_name(
                model_id,
                model_name,
